	...
	RE::BSEventNotifyControl animEventHandler::HookedProcessEvent(RE::BSAnimationGraphEvent& a_event, RE::BSTEventSource<RE::BSAnimationGraphEvent>* src)
	{
		...
		case "DragonPassByEffect"_h:
			if (DovahAI_Space::DovahAI::GetBoolVariable(a_actor, "bLDP_IsinCombat"))
			{
				...
				//Tx.x = -1.0f;
				// FXDragonTakeoffImpactSet [IPDS:00019A01]
				//GFunc_Space::GFunc::PlayImpactEffect(a_actor, RE::MakeFunctionArguments(std::bit_cast<RE::BGSImpactDataSet *>(FXDragonTakeOffImpactSet), std::bit_cast<const char *>(empty_str)), 0.0f, 0.0f, -1.0f, 512.0f, false, false);
			...
			if (DovahAI_Space::DovahAI::GetBoolVariable(a_actor, "bLDP_IsinCombat"))
			{
				...
				if (auto var = DovahAI_Space::DovahAI::GetIntVariable(a_actor, "iLDP_tailAttack_counter"))
				{
					if (var < 2){
						...
						GFunc_Space::GFunc::Call_Papyrus_Function(a_actor, "dragonActorSCRIPT", "LDP_PlayImpactEffect", RE::MakeFunctionArguments(std::bit_cast<RE::BGSImpactDataSet *>(FXDragonLandingImpactSet), std::bit_cast<const char *>(NPCTail8)));
		...
		case "Voice_SpellFire_Event"_h:
			...
			if (DovahAI_Space::DovahAI::GetBoolVariable(a_actor, "bLDP_IsinCombat"))
			{
				...
				// if (!DovahAI_Space::DovahAI::GetBoolVariable(a_actor, "bLDP_AIControl_doOnceOther") && !DovahAI_Space::DovahAI::IsUnSafeDragon(a_actor))
				// {
				// 	if (a_actor->AsActorState()->GetFlyState() == RE::FLY_STATE::kNone)
				// 	{
				// 		a_actor->SetGraphVariableBool("bLDP_AIControl_doOnceOther", true);
					...
					for (auto data : it->second)
					{
						...
						if (update)
						{
							...
							if (duration_cast<std::chrono::milliseconds>(time_now - time_initial).count() >= time_required.count())
							{
								...
								case "CDFAI_Wt1_Update"_h:
									if (auto targethandle = a_actor->GetActorRuntimeData().currentCombatTarget.get(); targethandle)
									{
										auto ct = targethandle.get();
										if (DovahAI_Space::DovahAI::GetIntVariable(a_actor, "iLDP_CDAIF_Var") <= DovahAI_Space::DovahAI::HoverWaitTime(a_actor) && GFunc_Space::IsAllowedToFly(a_actor, 1.0f) && ct && !ct->IsDead() && a_actor->GetPosition().GetDistance(ct->GetPosition()) <= 500.0f * 2.5f)
										{
											a_actor->SetGraphVariableInt("iLDP_CDAIF_Var", DovahAI_Space::DovahAI::GetIntVariable(a_actor, "iLDP_CDAIF_Var") + 1);
											std::tuple<bool, std::chrono::steady_clock::time_point, GFunc_Space::ms, std::string> Xt;
								...
								case "THAS_Wt1_Update"_h:
									if (auto targethandle = a_actor->GetActorRuntimeData().currentCombatTarget.get(); targethandle)
									{
										auto ct = targethandle.get();
										if (DovahAI_Space::DovahAI::GetIntVariable(a_actor, "iLDP_THAS_Var") <= DovahAI_Space::DovahAI::HoverWaitTime(a_actor) && GFunc_Space::IsAllowedToFly(a_actor, 1.0f) && ct && !ct->IsDead() && a_actor->GetPosition().GetDistance(ct->GetPosition()) <= 500.0f * 2.5f)
										{
											a_actor->SetGraphVariableInt("iLDP_THAS_Var", (DovahAI_Space::DovahAI::GetIntVariable(a_actor, "iLDP_THAS_Var") + 1));
											std::tuple<bool, std::chrono::steady_clock::time_point, GFunc_Space::ms, std::string> Bt;
